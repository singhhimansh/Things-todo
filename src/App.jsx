...
function App() {
  ...
  return (
    <div className="min-w-screen pt-10 pb-16  min-h-screen bg-gray-100" >
      <div className=" min-w-screen flex flex-col items-center">
        <div className="w-1/2 m-10   bg-gray-50 shadow-lg  shadow-indigo-500/40  border-solid border-2 border-gray-300">
          <div className="w-full flex flex-col items-center">
            <h1 className="flex my-10 font-semibold text-2xl">Things To Do</h1>
            <hr className="w-3/4 border-solid border-t-2 border-gray-300" />
            {/* Return fetched data */}
            <div className="my-5 flex flex-col w-[500px]">
              {
                ...
                Object.values(todoByID).map((item) => {
                  ...
                  return (
                    <div key={item.id} className="px-5 py-3 flex gap-5 ">
                      <input type="checkbox" className="cursor-pointer" id={`todo-item-${item.id}`} checked={item.completed} onChange={(e)=> handleToggleChecked(e,item.id)} />
                      <label className={` pb-1 w-11/12 flex-1 cursor-pointer ${item.completed ? 'line-through' : '' } `} htmlFor={`todo-item-${item.id}`} >{item.title}</label>
                ...
                }) : <div className="text-center text-sm italic text-gray-500"> Looks like you're absolutely free today. Why not go for a walk then! </div> 
              }
            </div>
            <hr className="w-3/4 border-solid border-t-2 border-gray-300" />
            ...
            <div className=" flex mt-10 mb-20 mx-6">
              {/* input manual tasks */}
              <input type="text" value={inputValue} onChange={(e)=>setInputValue(e.target.value)} className="px-4 h-10 w-80 mx-1 flex-1 border-2 border-solid border-gray-400 rounded-md" placeholder="Enter new task" name="tasktitle" id="task" />
